...
const createTaskItemElement = (text) => {
  ...
  const handleClickDeleteButtonImg = () => {
    $taskList.removeChild(taskItemLi);
  };

  deleteButtonImg.addEventListener('click', handleClickDeleteButtonImg);

  taskItemLi.appendChild(taskTextSpan).classList.add('add');
  taskItemLi.appendChild(deleteButtonImg).classList.add('deleteBtn');
